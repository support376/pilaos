...
function CardSkeleton() {
  return <div className="h-24 w-full shrink-0 animate-pulse rounded-lg bg-gray-100 sm:h-24 sm:w-32" />;
}

export function ListingCard({ listing: l, withPhoto = true }: { listing: Listing; withPhoto?: boolean }) {
  ...
  return (
    <Link href={`/listings/${l.id}`}
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start">
        {withPhoto ? (
          <Suspense fallback={<CardSkeleton />}>
            <PhotoMain kakaoPlaceId={l.studio.kakao_place_id} naverUrl={l.studio.naver_url} alt={l.studio.place_name} size="card" />
          </Suspense>
        ) : <CardSkeleton />}
      ...
      <div className="mt-3 flex flex-wrap items-center gap-1">
        ...
        {reviewSum > 0 ? (
          <span className="inline-flex items-center gap-1 rounded-full bg-gray-50 px-2 py-0.5 text-[10px] text-gray-700">★ 리뷰 {reviewSum.toLocaleString()}</span>
        ) : null}
      </div>
      ...
      <div className="mt-3 grid grid-cols-3 gap-2 sm:grid-cols-5">
        ...
        <Metric label="월수익률" value={`${l.estimate.monthly_yield_pct}%`} />
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-md bg-gray-50 px-2.5 py-1.5">
      <div className="flex items-center gap-1 text-[9px] uppercase tracking-wide text-gray-500">{label}<span className="text-amber-600">~</span></div>
