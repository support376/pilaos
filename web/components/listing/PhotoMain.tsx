...
export async function PhotoMain({ kakaoPlaceId, lng, lat, naverUrl, alt, size = "card", sigungu }: Props) {
  ...
  const dimensions = size === "hero"
    ? { containerClass: "relative h-64 w-full overflow-hidden bg-gray-100 sm:h-80", imgSizes: "(min-width: 768px) 768px, 100vw" }
    : { containerClass: "relative h-32 w-full overflow-hidden rounded-t-lg bg-gray-100", imgSizes: "(min-width: 768px) 360px, 100vw" };
