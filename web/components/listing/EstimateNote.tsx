...
export function EstimateNote({ listing, expandable = true }: { listing: Listing; expandable?: boolean }) {
  ...
  return (
    <details className="mt-3 rounded-lg border border-amber-200 bg-amber-50/60 p-3 text-xs text-amber-900" open={!expandable}>
      <summary className="cursor-pointer font-semibold">
        ⓘ 권리금·매출·순익은 모두 <strong>{confidenceLabel(e.confidence)}</strong> — 공개 데이터 기반 산식
      </summary>
      <div className="mt-2 space-y-1.5 text-amber-900/90 leading-relaxed">
        ...
        <p className="text-[11px] mt-2 text-amber-800/80">멀티플 {e.multiple_used}x · 신뢰 {Math.round(e.confidence_score * 100)}%</p>
      </div>
    </details>
  );
}
